...
description: "Job tokens die with their jobs. Deploy keys sprawl. So agents stopped pushing entirely — and got safer than both."
date: "2026-08-29"
author: tiny systems
---

Every agent platform eventually faces the same question: *how does the
agent push its work?* The obvious answers are all bad.
...
**Proxy the push through a server** and now you run a server — with the
credentials — which is the thing we set out to not do.

## What we shipped instead

The agent doesn't push. It can't; it has nothing to push with. When a
branch is ready, the session runs one git command that needs no network
and no secrets:
...
- The exec call is argv-only with an allowlist on bundle names — no shell,
  no interpolation.
...
The first pull request grown this way is up in our demo garden:
[seedling PR #2](https://github.com/tiny-systems/seedling/pull/2) —
+100/−3, sown by labeling an issue, harvested by the courier.
